...
2. **Transcription Strategy**: Uses the Groq API (`whisper-large-v3`) to process audio stored in the shared volume. API requests are throttled (rate-limited) according to the configured threshold to prevent exceeding usage limits. It employs a dual-output strategy:
